        ...
        <div className="md:grid lg:grid-cols-4  md:grid-cols-2 flex flex-col justify-center items-center gap-5 mx-auto md:w-[90%] lg:w-[90%] ">
            {
                accessories_list.map((item, index) => {
                    if (category === "All" || category === item.category) {
                        return <AccessoriesItem key={index} id={item?._id} name={item?.name} description={item?.description} image={item?.image} price={item?.price} category={item?.category}></AccessoriesItem>
                    }
